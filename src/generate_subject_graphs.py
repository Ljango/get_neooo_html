...
def generate_geography_graph():
    ...
    # 生成Neo4j脚本
    generator.generate_neo4j_cypher(
        str(DATA_ROOT / "义教地理" / "neo4j_import.cypher"),
        title="义教地理课标图谱"
    )
    
    print(f"\n✅ 义教地理图谱生成完成")
    print(f"   - HTML: static/义教地理_课标.html")
    print(f"   - Neo4j: 图谱数据/义教地理/neo4j_import.cypher")
...
def generate_highschool_chemistry_graph():
    ...
    # 生成Neo4j脚本
    generator.generate_neo4j_cypher(
        str(DATA_ROOT / "高中化学" / "neo4j_import.cypher"),
        title="高中化学课标图谱"
    )
    
    print(f"\n✅ 高中化学图谱生成完成")
    print(f"   - HTML: static/高中化学_课标.html")
    print(f"   - Neo4j: 图谱数据/高中化学/neo4j_import.cypher")
...
def generate_highschool_physics_graph():
    ...
    generator.generate_html(
        "高中物理_课标.html",
        title="高中物理课标图谱",
        description="高中阶段物理课程标准完整知识结构"
    )
    
    # 生成Neo4j脚本
    generator.generate_neo4j_cypher(
        str(DATA_ROOT / "高中物理" / "neo4j_import.cypher"),
        title="高中物理课标图谱"
    )
    
    print(f"\n✅ 高中物理图谱生成完成")
    print(f"   - HTML: static/高中物理_课标.html")
    print(f"   - Neo4j: 图谱数据/高中物理/neo4j_import.cypher")
