...
├── src/
│   ├── config.py               # 🆕 统一配置文件（含学科Neo4j标签）
│   ├── graph_generator.py      # 核心：图谱生成器
│   ├── neo4j_importer.py       # 🆕 Neo4j数据导入模块
...
├── static/                      # 生成的HTML文件
├── 图谱数据/                    # 原始数据
├── import_data.py              # 向后兼容入口
└── env.template                # Neo4j环境配置模板
...
- **config.py** - 统一配置中心
  ...
  - 学科配置（SUBJECT_CONFIG）- 含Neo4j分类标签
  - 服务器配置
...
- **manage.py** - 命令行管理工具（🆕推荐）
  - 整合所有功能的统一入口
  - 支持：generate, import, list, update-index, serve
...
python3 manage.py import --subject 高中数学 --clear  # 清除旧数据后导入
...
```cypher
# 查询高中数学的知识点
MATCH (n:GaoZhongShuXue:KeyPoint) RETURN n

# 查询某学科的完整图谱
MATCH (n:YiJiaoWuLi)-[r]->(m:YiJiaoWuLi) RETURN n, r, m
```
...
"义教英语": {
    ...
    "files": ["义教英语_课标.html"],
    "color": "#3498db",
    "neo4j_label": "YiJiaoYingYu"  # Neo4j分类标签
}
```
...
# 导入到Neo4j（可选）
python3 manage.py import --subject 义教英语
```
